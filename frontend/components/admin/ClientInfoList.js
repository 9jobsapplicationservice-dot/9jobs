...
import { Search, Download, Eye, X, Calendar, MapPin, Briefcase, DollarSign, Clock, Shield, Mail, Edit3, Trash2 } from 'lucide-react';
...
export default function ClientInfoList({ initialSubmissions }) {
  ...
  const filteredSubmissions = submissions.filter((sub) => {
    ...
    return (
      ...
      sub.contactNo.toLowerCase().includes(term) ||
      sub.preferredRole.toLowerCase().includes(term)
    );
  });
  ...
  const handleSaveEdit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSubmissions((prev) => prev.map((sub) => (sub._id === updated._id ? updated : sub)));
        setEditingSub(null);
      ...
      console.error(err);
      alert('An error occurred while updating submission.');
    }
  };
    ...
    <section className="admin-panel" style={{ width: '100%' }}>
      <div className="admin-panel__header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p>Manage and download information and resumes submitted by prospective clients.</p>
        </div>
          ...
          <table className="admin-table">
            ...
            <tbody>
              {filteredSubmissions.map((sub) => (
                <tr key={sub._id}>
                  <td style={{ whiteSpace: 'nowrap' }}><strong>{sub.fullName}</strong></td>
                  <td style={{ whiteSpace: 'nowrap' }}>{sub.email}</td>
                  ...
                  <td style={{ whiteSpace: 'nowrap' }}>
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <button
                        ...
                        style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '4px 8px', fontSize: '0.82rem' }}
                      >
                        <Eye size={13} /> View
                      </button>
                      <button
                        ...
                        style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '4px 8px', fontSize: '0.82rem', color: '#2563eb' }}
                      >
                        <Edit3 size={13} /> Edit
                      </button>
                      <button
                        ...
                        style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '4px 8px', fontSize: '0.82rem', color: '#dc2626' }}
                      >
        ...
        >
          ...
          >
            ...
            <div style={{ marginBottom: '1.5rem', borderBottom: '1px solid var(--color-border)', paddingBottom: '1rem' }}>
              ...
              <p style={{ fontSize: '0.9rem', color: 'var(--color-text-muted)' }}>Modify the candidate details below.</p>
            </div>
            ...
            <form onSubmit={handleSaveEdit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.2rem' }}>
                ...
                <div>
                  ...
                  <input
                    required
                    type="text"
                    value={editingSub.contactNo}
                    onChange={(e) => updateEditField('contactNo', e.target.value)}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--color-border)', borderRadius: '6px' }}
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.2rem' }}>
                ...
                <div>
                  ...
                  <input
                    required
                    type="text"
                    value={editingSub.preferredRole}
                    onChange={(e) => updateEditField('preferredRole', e.target.value)}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--color-border)', borderRadius: '6px' }}
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.2rem' }}>
                <div>
                  ...
                  <select
                    value={editingSub.workingRights}
                    onChange={(e) => updateEditField('workingRights', e.target.value)}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--color-border)', borderRadius: '6px', background: 'var(--color-bg)' }}
                  >
              ...
              {editingSub.workingRights === 'Other' && (
                <div>
                  <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: '600', marginBottom: '4px' }}>Specify Working Rights *</label>
                  <input
                    required
                    type="text"
                    value={editingSub.workingRightsCustom}
                    onChange={(e) => updateEditField('workingRightsCustom', e.target.value)}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--color-border)', borderRadius: '6px' }}
                  />
                </div>
              )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.2rem' }}>
                <div>
                  ...
                  <input
                    required
                    type="text"
                    value={editingSub.expectedSalary}
                    onChange={(e) => updateEditField('expectedSalary', e.target.value)}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--color-border)', borderRadius: '6px' }}
                  />
                </div>
                <div>
                  ...
                  <input
                    required
                    type="text"
                    value={editingSub.noticePeriod}
                    onChange={(e) => updateEditField('noticePeriod', e.target.value)}
                    style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--color-border)', borderRadius: '6px' }}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  required
                  type="text"
                  value={editingSub.preferredJobLocation}
                  onChange={(e) => updateEditField('preferredJobLocation', e.target.value)}
                  style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--color-border)', borderRadius: '6px' }}
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--color-border)', borderRadius: '6px', minHeight: '80px' }}
                />
              </div>
      ...
      {selectedSub && (
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '12px', borderTop: '1px solid var(--color-border)', paddingTop: '1.5rem', marginTop: '1.5rem' }}>
              <a
                className="admin-primary-button"
