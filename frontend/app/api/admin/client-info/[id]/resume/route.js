...
export async function GET(request, { params }) {
  try {
    ...
    if (!clientInfo || !clientInfo.resumeStorageKey) {
    ...
    const { stream, file } = await openDownloadStreamByKey(clientInfo.resumeStorageKey);
    const contentType = file.contentType || 'application/octet-stream';
    const fileName = clientInfo.resumeFileName || `resume-${id}.pdf`;
