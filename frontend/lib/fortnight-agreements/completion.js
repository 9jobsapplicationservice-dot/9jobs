...
import { sendAgreementCompletedEmail } from '@/lib/fortnight-agreements/email';
...
export async function executeFinalSealing(agreement) {
  ...
  const lockedAgreement = await FortnightAgreement.findOneAndUpdate(
    {
      _id: agreement._id,
  ...
  try {
    ...
    const signedUpload = await uploadPrivatePdf({
      folder: `fortnight-agreements/${lockedAgreement._id}`,
    ...
    await FortnightAgreement.updateOne(
...
export async function retryFailedAgreementCompletion(agreementId) {
  ...
  const agreement = await FortnightAgreement.findOneAndUpdate(
    {
      _id: agreementId,
      status: 'completion_processing_failed',
    },
  ...
  if (!agreement) {
    const existingAgreement = await FortnightAgreement.findById(agreementId);
  ...
  return FortnightAgreement.findById(agreementId);
}
