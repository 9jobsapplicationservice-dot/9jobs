...
jest.doMock('@/lib/fortnight-agreements/service', () => ({
...
async function loadRoute() {
  jest.resetModules();
  return import('@/app/api/fortnight-agreements/[id]/send/route');
}
...
function makeAgreement(overrides = {}) {
  return {
    ...
    generatedPdfPath: 'fortnight/agree-1/generated.pdf',
    originalPdfSha256: '',
    originalPdfUrl: '',
    ...
    signedPdfPath: 'fortnight/agree-1/signed.pdf',
...
describe('fortnight send route', () => {
  ...
  test('restarts the workflow in a clean client-first state before sending', async () => {
    const agreementDocument = makeAgreement();
    mockGetAgreementDocumentById.mockResolvedValue(agreementDocument);

    const { POST } = await loadRoute();
    const response = await POST(new Request('http://localhost/api/fortnight-agreements/agreement-1/send', { method: 'POST' }), {
    ...
    expect(agreementDocument.clientTokenUsedAt).toBeNull();
    ...
    expect(agreementDocument.providerTokenUsedAt).toBeNull();
  ...
  test('blocks sending once the workflow has already started', async () => {
    mockGetAgreementDocumentById.mockResolvedValue(makeAgreement({ status: 'sent_to_provider' }));
